...
def sign_up():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first() # checking if user(email) exists
        if user:
            flash('Email aleardy exists.', category='error')
        elif len(email) < 4:
            ...
            db.session.commit()
            login_user(new_user, remember=True) #using login_user object from flask_login module 
...
def login():
    ...
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first() #query the database, filter users by email and return first result
        if user:
